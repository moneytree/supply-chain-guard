...
function parsePackageJson(logger, manifestPath, packageInfo) {
  ...
  // Linked packages packages that are part of workspaces have the resolved field, but it's not a URL.
  if (!packageInfo.resolved || packageInfo.inBundle || packageInfo.link) {
    return null;
  }
...
const parseManifest = async (manifestPath) => {
  // a dedicated function to ensure nothing holds onto the contents of the manifest file longer than necessary,
  // since it can be large and we want to free memory as soon as possible
