...
//   https://github.com/moneytree/mt-some-github-repo.git#commit=dd0df140a0e5eb3f6384f11d7bae3decac9afae7
...
const createPackageFromPlainUrl = (manifestPath, url) => {
  ...
  let hash = url.hash.substring(1); // remove leading '#'
  if (hash.startsWith('commit=')) {
    hash = hash.substring('commit='.length);
  }
